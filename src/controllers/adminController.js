...
import Bid from '../models/Bid.js';
import VerificationLog from '../models/VerificationLog.js';
import { createNotification } from './notificationController.js'; // 🔔 Notification import
...
// 🔔 NEW FUNCTION - TASK APPROVAL WITH NOTIFICATION
export const updateTaskApproval = async (req, res) => {
    try {
        ...
        await createNotification(task.userId, message, 'task', `/my-tasks`);

        res.status(200).json({ message: `Task ${decision} successfully`, task });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
